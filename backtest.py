...
from pearson import (
    build_dataset, get_feature_columns, REPORTS_DIR,
    make_synthetic_ohlcv, add_technical_indicators,
)

ALL_MODEL_FEATURES_NO_SENTIMENT = get_feature_columns()
...
def walk_forward_backtest(df, feature_cols, min_train_size=100, retrain_every=20):
    ...
    if n <= min_train_size + 5:
        raise ValueError(f"Not enough rows ({n}) to backtest with min_train_size={min_train_size}")
    ...
    for i in range(min_train_size, n - 1):
        ...
        actual_close_tomorrow = float(df["Close"].iloc[i + 1]) if i + 1 < n else actual_close_today
        actual_return = (actual_close_tomorrow - actual_close_today) / actual_close_today
...
def compute_backtest_metrics(results_df):
    ...
    cumulative_buy_hold_return = float(np.prod(1 + results_df["actual_return_next_day"]) - 1)

    win_rate_when_predicted_up = None
    up_preds = results_df[results_df["predicted"] == 1]
    if len(up_preds) > 0:
        win_rate_when_predicted_up = float((up_preds["actual_return_next_day"] > 0).mean())
    ...
    return {
        ...
        "high_confidence_accuracy": round(float(high_conf_accuracy), 4) if high_conf_accuracy is not None else None,
        "cumulative_strategy_return_pct": round(cumulative_strategy_return * 100, 2),
...
def export_backtest_csv(backtest_output, out_path=None):
    ...
    out_path = out_path or os.path.join(REPORTS_DIR, f"{ticker.replace('-', '_')}_backtest.csv")
...
def format_backtest_summary(backtest_output):
    ...
    lines.append(f"Predictions made: {m['total_predictions']}")
...
def test_rolling_accuracy_adds_column():
    ...
    feature_cols = get_feature_columns()
    results = walk_forward_backtest(df, feature_cols, min_train_size=100, retrain_every=20)
    results = rolling_accuracy(results, window=20)
    assert "rolling_accuracy" in results.columns


def test_too_little_data_raises_error():
    df = make_synthetic_backtest_dataset(n=80)
    feature_cols = get_feature_columns()
    try:
...
def run_backtest_tests():
    tests = [
        test_walk_forward_backtest_produces_expected_row_count,
        test_backtest_metrics_have_required_keys,
        test_accuracy_between_zero_and_one,
    ...
    print(f"\n{passed}/{len(tests)} backtest tests passed")
...
if __name__ == "__main__":
    ...
    if args.test:
        ...
        output = run_ticker_backtest(args.ticker, period=args.period, use_cache=not args.no_cache)
        print(format_backtest_summary(output))
        path = export_backtest_csv(output)
